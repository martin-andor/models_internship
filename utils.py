...
def load_microsoft_ds(path):
    ...
    sents1 = elmonize(sents1, embedding_model)
    sents2 = elmonize(sents2, embedding_model)
    print(f'Shape of elmonized sentences 1: {sents1.shape}')
    print(f'Shape of elmonized sentences 2: {sents2.shape}')

    sents1 = tf.expand_dims(tf.constant(sents1, dtype=tf.float32),axis=1)
    sents2 = tf.expand_dims(tf.constant(sents2, dtype=tf.float32),axis=1)
